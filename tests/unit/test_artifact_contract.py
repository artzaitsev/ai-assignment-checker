...
def test_stage_artifact_keys_cover_all_stage_lifecycles() -> None:
    ...
    for stage, keys in STAGE_ARTIFACT_KEYS.items():
...
@pytest.mark.unit
def test_llm_output_stage_contains_feedback_artifact() -> None:
    assert artifact_keys_for_stage(stage="llm-output") == ("llm-output", "feedback")


@pytest.mark.unit
def test_put_artifact_ref_rejects_unknown_key() -> None:
    with pytest.raises(ValueError, match="unsupported artifact key"):
        put_artifact_ref(artifacts={}, key="unknown", artifact_ref="stub://unknown/ref")
