...
_PROCESSED_RE = re.compile(r"^processed\s+(\d+)\s+telegram updates")


class TelegramPollingWorkerLoop(WorkerLoop):
    async def run_once(self) -> bool:
        # Telegram ingest is transport-facing: it polls Telegram and creates
        # `uploaded` submissions directly, so there is no DB claim to acquire.
